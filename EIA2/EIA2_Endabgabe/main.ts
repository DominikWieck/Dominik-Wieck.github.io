namespace Picture {
    ...
    function createFigure(_event: MouseEvent): void {
        let position: Vector = new Vector(_event.clientX - crc2.canvas.offsetLeft, _event.clientY - crc2.canvas.offsetTop); // Mausposition wird genutzt 
    ...
    async function restorePicture(_event: Event): Promise<void> {
        if (list == "") {
            //Anfrage senden
